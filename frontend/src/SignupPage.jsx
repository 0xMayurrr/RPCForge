...
export default function SignupPage() {
  ...
  const handleSubmit = (e) => {
    ...
    // Simulate real SAAS network delay
    setTimeout(() => {
      navigate('/dashboard');
    }, 1800);
  };
    ...
    <div className="min-h-screen bg-background-dark flex flex-col justify-center items-center p-6 text-slate-100 font-display selection:bg-primary/30">
      ...
      <div className="w-full max-w-md bg-obsidian border border-border-subtle rounded-2xl p-8 shadow-2xl">
        <h1 className="text-2xl font-bold text-white mb-2 text-center">
          {plan === 'pro' ? 'Start your Pro Trial' : 'Create an account'}
        </h1>
        <p className="text-sm text-slate-400 text-center mb-8">
          {plan === 'pro' 
            ? 'Sign up to get your advanced API key with priority support.' 
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          ...
          <button 
            type="submit" 
            disabled={isLoading}
            className="w-full py-2.5 mt-2 bg-primary hover:bg-primary/90 text-white font-semibold rounded-lg shadow-[0_0_15px_rgba(100,103,242,0.4)] transition-all flex items-center justify-center gap-2 disabled:bg-primary/50 disabled:shadow-none"
          >
        ...
        <button 
          ...
          type="button" 
          disabled={isLoading}
          className="w-full flex items-center justify-center gap-3 py-2.5 bg-white/5 hover:bg-white/10 border border-white/10 text-white font-semibold rounded-lg transition-colors disabled:opacity-50 disabled:hover:bg-white/5"
        >
